      ...
      // 2. interporlate to grids with shift
        ...
        for (int gz=0; gz<arr->nz; gz++) for (int gy=0; gy<arr->ny; gy++) for (int gx=0; gx<arr->nx; gx++){
            ...
            if (gx>=0 && gx<nx && gy>=0 && gy<ny && gz>=0 && gz<nz){
                ...
                ig1[0] = gx; ig2[0] = ig1[0] + 1; w2[0] = shift_x; w1[0] = 1 - shift_x; if (ig1[0]<0 || ig1[0]>=nx || ig2[0]<0 || ig2[0]>=nx) in_box = false;
                ig1[1] = gy; ig2[1] = ig1[1] + 1; w2[1] = shift_y; w1[1] = 1 - shift_y; if (ig1[1]<0 || ig1[1]>=ny || ig2[1]<0 || ig2[1]>=ny) in_box = false;
                ig1[2] = gz; ig2[2] = ig1[2] + 1; w2[2] = shift_z; w1[2] = 1 - shift_z; if (ig1[2]<0 || ig1[2]>=nz || ig2[2]<0 || ig2[2]>=nz) in_box = false;
                if (in_box){
    ...
    void calculate_solvent_ef(IET_Param * sys, IET_arrays * arr, double gamma){
      ...
      // calculate the electrostatic field generated by
        double shift_x = arr->box.x / arr->nx / 1000;
        double shift_y = arr->box.y / arr->ny / 1000;
        double shift_z = arr->box.z / arr->nz / 1000;
