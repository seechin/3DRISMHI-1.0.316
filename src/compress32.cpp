...
const char * szCompressPageHeader = "TS4S";
class IETSPageHeader {
  public:
    char identifier[4]; // must be "TS4S"
    char title[4]; // can be: g/guv, h/huv/hsr/hlr, c/cuv/csr/clr, lj/coul, uuv/ulr, ...
    unsigned int crc32; // CRC32 of stored data
    unsigned int data_offset; // begin of data from begin of this header
    unsigned int data_length; // length of data
    unsigned int data_original_length; // length of original data, data_length=data_original_length implies no compression
...
class CompressPageHeader : public IETSPageHeader {
  public:
    unsigned short dimensions[4]; // nx, ny, nz, nv: from low to high dimension
    double time_stamp; // frame number of frame time
};

void get_version_array_from_string(unsigned short array[4], const char * string){
    char string_buffer[128]; memset(string_buffer, sizeof(string_buffer), 0);
    ...
    if ((header->compressor_version[0]&0xFF)=='u' || (header->compressor_version[0]&0xFF)=='U'){
        ...
        while (pd<data+header->data_original_length && pc<compressed+header->data_length){
          // uncompress this page
            size_t size_c_page = *(int*)pc; pc += 4;
...
size_t write_data_page(FILE * fo, const char * filename, double time_stamp, const char * title, FILE * flog, unsigned short dimensions[4], void * data, size_t data_size, StringNS::string comment, int compress_level, size_t page_size){
    if (!title) title = "";
    if (!fo) return 0;
  ...
  // compress data and do CRC
    ...
    if (compress_level!=0){
      ...
      // compress by blocks
        int compressRet = 0; size_t last_compressedlen = 0;
        for (size_t begin=0; begin<data_size; begin+=page_size){
            size_t * this_blk_size = (size_t *) &compressBuf[last_compressedlen]; last_compressedlen += sizeof(unsigned int); header.data_length += sizeof(unsigned int);
            size_t originallen = begin+page_size<data_size? page_size : data_size-begin;
            if (originallen+last_compressedlen>allocate_memory_size){ header.data_length = 0; compressRet = -1; break; }
            size_t compressedlen = originallen;
      ...
      // compress all together
        //size_t compressedlen = data_size;
