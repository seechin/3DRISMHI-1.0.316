      ...
      */
        for (int iv=0; iv<sys->nv; iv++){
            for (size_t i3=0; i3<N3; i3++){
                //arr->huv[iv][0][0][i3] += arr->hlr[iv][0][0][i3] + (sys->nbulk_rism[sys->av[iv].iaa] - 1);
      ...
      // step 2: RISM loop
        ...
        for (int istep=0; istep<sys->stepmax_rism; istep++){
          ...
          // 2. DIIS step in
            ...
            lap_timer_diis();
          // 3. report and loop control
            bool stop_loop = report_3drism_convergence(flog, sys, arr, err, success, closure_buffer, istep);
            if (stop_loop) break;
        }
